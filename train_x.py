...
def parse_args():
    ...
    parser.add_argument('--tb_dir', type=str, default='tensorboard_logs_X/temp')
...
def train(train_loader, cfg):
    ...
    for idx_epoch in range(cfg.n_epochs):
        scheduler.step()
        for idx_iter, (HR_left, HR_right, LR_left, LR_right) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {idx_epoch+1}"):
            ...
            ### loss_SR
            loss_SR = (criterion_mse(SR_left, HR_left) + criterion_mse(SR_right, HR_right)) / 2
            ...
            psnr = cal_psnr(HR_left[:,:,:,64:].data.cpu(), SR_left[:,:,:,64:].data.cpu())
        ...
        if idx_epoch % 1 == 0:
            with torch.no_grad():
                ...
                filename = f"PASSRnet_bs{cfg.batch_size}_epoch{idx_epoch + 1}.pth.tar"
                save_ckpt({
                    'epoch': idx_epoch + 1,
                    'state_dict': net.state_dict(),
                    'loss': loss_list,
                    'psnr': psnr_list,
                }, save_path = 'log_x/', filename=filename)
