...
def parse_args():
    ...
    parser.add_argument('--pretrained_path', type=str, default='log/fixed_mask/PASSRnet_epoch25.pth.tar', help='path to pretrained model')
...
def train(train_loader, cfg):
    ...
    if cfg.pretrained_path is not None and os.path.exists(cfg.pretrained_path):
        print(f"Loading pretrained model from {cfg.pretrained_path}")
        checkpoint = torch.load(cfg.pretrained_path)
        net.load_state_dict(checkpoint['state_dict'], strict=False)

        # Freeze feature extractor and PAM
        for param in net.init_feature.parameters():
            param.requires_grad = False
        for param in net.pam.parameters():
            param.requires_grad = False
    ...
    for idx_epoch in range(cfg.n_epochs):
        scheduler.step()
        for idx_iter, (_, HR_right, LR_left, LR_right) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {idx_epoch+1}"):
            b, c, h, w = LR_left.shape
            HR_right, LR_left, LR_right  = Variable(HR_right).to(cfg.device), Variable(LR_left).to(cfg.device), Variable(LR_right).to(cfg.device)
        ...
        if idx_epoch % 1 == 0:
            ...
            save_ckpt({
                ...
                'loss': loss_list,
                'psnr': psnr_list,
            }, save_path = 'log_right/', filename='PASSRnet' + '_epoch' + str(idx_epoch + 1) + '_fixed_pretrained.pth.tar')
            psnr_epoch = []
            loss_epoch = []
