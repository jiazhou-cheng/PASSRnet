...
def test(test_loader, cfg):
    ...
    checkpoint = torch.load('log_onlyR/PASSRnet_epoch25_fixed_mse.pth.tar', map_location=torch.device('cuda:0'))
    net.load_state_dict(checkpoint['state_dict'])

    psnr_list = []

    with torch.no_grad():
        for idx_iter, (_, HR_right, LR_left, LR_right) in enumerate(test_loader):
            ...
            SR_right = net(LR_right, LR_right, is_training=0)
            SR_right = torch.clamp(SR_right, 0, 1)

            psnr_list.append(cal_psnr(HR_right[:,:,:,64:], SR_right[:,:,:,64:]))
            ...
            ## save results
            output_dir = f'results_onlyR/{cfg.dataset}/{scene_name}'
            os.makedirs(output_dir, exist_ok=True)
            SR_left_img = transforms.ToPILImage()(torch.squeeze(SR_right.data.cpu(), 0))
